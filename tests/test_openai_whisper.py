...
class TestOpenAIWhisperTranscriber(unittest.TestCase):
    ...
    def test_transcribe_with_openai_api(
        self, mock_file, mock_openai_class, mock_getsize, mock_exists, mock_makedirs
    ):
    ...
    @patch("os.path.getsize")
    @patch("openai.OpenAI")
    def test_transcribe_with_options(
        self, mock_openai_class, mock_getsize, mock_exists, mock_makedirs
    ):
        ...
        self.assertEqual(
            call_args["prompt"], "This is a podcast episode about technology."
        )
    ...
    def test_transcribe_with_timestamps(
        self, mock_openai_class, mock_exists, mock_makedirs
    ):
    ...
    @patch("os.path.getsize")
    @patch("openai.OpenAI")
    def test_transcribe_large_file_chunking(
        self, mock_openai_class, mock_getsize, mock_exists, mock_makedirs
    ):
    ...
    @patch("os.path.getsize")
    @patch("openai.OpenAI")
    def test_transcribe_with_different_models(
        self, mock_openai_class, mock_getsize, mock_exists, mock_makedirs
    ):
...
class TestTranscriptFetcherIntegration(unittest.TestCase):
    ...
    @patch(
        "ekko_prototype.pages.tools.openai_whisper_transcriber.OpenAIWhisperTranscriber"
    )
    def test_unified_fetcher_youtube_fallback_to_openai(
        self,
        mock_whisper_class,
        mock_youtube_class,
