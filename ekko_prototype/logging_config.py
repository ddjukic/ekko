    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        for key, value in record.__dict__.items():
            if key not in ['name', 'msg', 'args', 'created', 'filename', 'funcName',
                          'levelname', 'lineno', 'module', 'exc_info', 'exc_text',
                          'stack_info', 'pathname', 'processName', 'process',
                          'threadName', 'thread', 'getMessage', 'levelno', 'msecs',
                          'relativeCreated']:
                log_obj[key] = value
...
def setup_logging(
    ...
    logger.info(f"Logging configured - Level: {log_level}, JSON: {use_json}, File: {log_file}")
