...
class SimpleAuth:
        ...
        if self.user_data_file.exists():
            try:
                ...
                if st.session_state.session_id in data.get('sessions', {}):
                    ...
                    st.session_state.transcript_count = session.get('transcript_count', 0)
                    st.session_state.authenticated = session.get('authenticated', False)
                    
                    # Check if we need to reset daily limit
                    last_reset = datetime.fromisoformat(session.get('last_reset', datetime.now().isoformat()))
    ...
    def _save_user_data(self) -> None:
        ...
        try:
            ...
            if st.session_state.user_email:
                ...
                data['users'][st.session_state.user_email]['last_seen'] = datetime.now().isoformat()
                data['users'][st.session_state.user_email]['total_transcripts'] = (
                    data['users'][st.session_state.user_email].get('total_transcripts', 0) + 
    ...
    def login_form(self) -> bool:
        ...
        with st.container():
            ...
            st.markdown("Please enter your email to continue. You'll get **2 free transcript generations** as a demo user.")
    ...
    def display_usage_info(self) -> None:
        ...
        if st.session_state.authenticated:
            ...
            if within_limit:
                ...
                progress = st.session_state.transcript_count / DEMO_USER_TRANSCRIPT_LIMIT
                st.sidebar.progress(progress)
            else:
                st.sidebar.error("Daily transcript limit reached!")
                st.sidebar.markdown("Come back tomorrow for more free transcripts, or contact us for unlimited access.")
