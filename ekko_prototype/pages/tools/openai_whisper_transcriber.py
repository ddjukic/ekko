...
class OpenAIWhisperTranscriber:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "OpenAI API key required. Provide via api_key or credentials_file."
            )
    ...
    def transcribe(
        ...
        try:
            ...
            if file_size > self.MAX_FILE_SIZE:
                logger.warning(
                    f"File size {file_size} exceeds limit, will need chunking"
                )
                return self._transcribe_large_file(
                    audio_file_path, language, prompt, temperature
                )
            ...
            with open(audio_file_path, "rb") as audio_file:
                ...
                transcript_path = self._save_transcript(
                    transcript_text, audio_file_path
                )
        ...
        try:
            ...
            with open(audio_file_path, "rb") as audio_file:
                ...
                result = (
                    ...
                    else {"text": response.text, "segments": []}
                )

                logger.info(
                    f"Transcribed with {len(result.get('segments', []))} segments"
                )
                return result
    ...
    def _transcribe_large_file(
        ...
        logger.error(
            "Audio file is too large for OpenAI Whisper API. Please use a shorter episode or enable local transcription."
        )
    ...
    def translate(self, audio_file_path: str, prompt: str | None = None) -> str | None:
        """
        ...
        try:
            ...
            with open(audio_file_path, "rb") as audio_file:
                params = {"model": self.model, "file": audio_file}
                ...
                logger.info(
                    f"Successfully translated {len(translated_text)} characters"
                )
