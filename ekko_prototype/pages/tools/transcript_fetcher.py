...
class UnifiedTranscriptFetcher:
    ...
    def get_transcript(
        ...
        if self.config.prefer_youtube and podcast_rss_url:
            ...
            result = self._try_youtube_transcript(podcast_rss_url, episode_title)
        ...
        if not result or not result.text:
            if episode_audio_url:
                ...
                result = self._try_whisper_transcript(
                    episode_audio_url, episode_title, podcast_name
                )
            else:
    ...
    def _try_youtube_transcript(
        self, podcast_rss_url: str, episode_title: str
        ...
        try:
            ...
            is_available, youtube_url = (
                self.youtube_detector.check_youtube_availability(
                    podcast_rss_url, episode_title
                )
            )
    ...
    def _try_whisper_transcript(
        self, audio_url: str, episode_title: str, podcast_name: str
    ) -> TranscriptResult:
        """
        ...
        try:
            ...
            local_audio_path = self.episode_downloader.download_single_episode(
                audio_url, episode_title, podcast_name
            )
            ...
            if self.config.use_openai_whisper:
                ...
                if transcript_text:
                    ...
                    transcript_path = os.path.join(
                        "./transcripts", f"{episode_title}.txt"
                    )
                ...
                if self.audio_transcriber:
                    transcript_path = self.audio_transcriber.transcribe(
                        local_audio_path
                    )
                    if transcript_path and os.path.exists(transcript_path):
                        with open(transcript_path, encoding="utf-8") as f:
            ...
            if transcript_text:
                ...
                quality_score = self.youtube_detector.calculate_quality_score(
                    transcript_text
                )
                ...
                if self.config.use_openai_whisper:
                    ...
                    )  # Consider adding WHISPER_OPENAI
                elif self.config.use_remote_whisper:
    ...
    def _transcribe_remote(
        ...
        ngrok_url = os.getenv(
            "NGROK_URL", "https://internally-next-serval.ngrok-free.app"
        )
    ...
    def _check_cache(
        self, podcast_name: str, episode_title: str
    ) -> TranscriptResult | None:
        """
    ...
    def _cache_transcript(
        self, podcast_name: str, episode_title: str, result: TranscriptResult
    ) -> None:
        """
    ...
    def _get_cache_path(self, podcast_name: str, episode_title: str) -> Path:
        ...
        safe_podcast = "".join(
            c if c.isalnum() or c in " -_" else "_" for c in podcast_name
        )
        safe_episode = "".join(
    ...
    def _manage_cache_size(self) -> None:
        ...
        try:
            ...
            if total_size > max_size:
                ...
                cache_files = sorted(
                    self.cache_dir.rglob("*.json"), key=lambda f: f.stat().st_mtime
                )
